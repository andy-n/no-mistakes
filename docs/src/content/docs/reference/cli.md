...
Only attached runs receive plan-aware guidance: compare the proposed scenarios and independent expected results against actual evidence, prefer repeatable product verification with a retained artifact when no existing check drives a scenario, or name the missing capability and mark it untested. Follow repository testing rules before changing permanent tests. A missing-test finding must identify the observable failure, why existing checks and product evidence do not cover it, and the independent expected result. Runs without a plan retain their existing Review and Test guidance.
